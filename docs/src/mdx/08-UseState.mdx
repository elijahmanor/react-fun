...
If you have experience with Class Components, then you may be familiar that `this.setState()` merges any changes passed to it with it's current value. However, the `useState` updater does not do that.
...
**NOTE**: The following code is oversimplified, but intended to convey the idea of what is going on behind the scenes.
